    ...
    async getGraphdataRating() {
      ...
      .then(data =>{
          ...
          data.forEach(voting_stuff => {
            if(voting_stuff.voting !== '' &&  voting_stuff.voting !== 0){
              row = {
                vote: voting_stuff.foodname+ ' ' + ((voting_stuff.scheduled_date).toString()).replace("T21:00:00.000Z",""), 
                Äänet: voting_stuff.voting, 
                pv: 1400, 
                amt: 1400
              }
              data_array.push(row);
      ...
      console.log(data_array);
        //add data to array graphdata
      this.setState({graphdataRating:data_array})
    }
      ...
      return (
        <div style={{marginLeft: '20px'}}>
           ...
           {/* <LineChart width={this.state.graphWidth} height={300} data={data} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
            ...
            <BarChart width={this.state.graphWidth} height={300} data={this.state.graphdataRating}>
                <XAxis dataKey="vote" stroke="#8884d8" />
                <YAxis />
                <Tooltip wrapperStyle={{ width: 100, backgroundColor: '#ccc' }} />
                <Legend width={100} wrapperStyle={{ top: 40, right: 20, backgroundColor: '#f5f5f5', border: '1px solid #d5d5d5', borderRadius: 3, lineHeight: '40px' }} />
                <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
                <Bar type="monotone" dataKey="Äänet" fill="#8884d8" barSize={30} />
            </BarChart>
            {/* </div> */}
        </div>
      );
